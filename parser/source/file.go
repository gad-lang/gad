...
func (f *SourceFile) SafePosition(p Pos) (pos SourceFilePos) {
	offset := int(p) - f.Base
	pos.Offset = offset
...
func (f *SourceFile) LineIndexOf(p Pos) int {
	l := len(f.Lines)
	for i := l; i > 0; i-- {
		p2 := f.Lines[i-1]
		if p2 <= int(p) {
...
func (f *SourceFile) LinePos(p Pos) Pos {
	l := len(f.Lines)
	for i := l; i > 0; i-- {
		p2 := f.Lines[i-1]
		if p2 <= int(p) {
...
func (f *SourceFile) NextLinePos(p Pos) Pos {
	l := len(f.Lines)
	for i := l; i > 0; i-- {
		p2 := f.Lines[i-1]
		if p2 <= int(p) {
