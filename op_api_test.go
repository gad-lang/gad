...
func TestBinOpObjectDispatch(t *testing.T) {
	...
	ret, err, handled := binOpObject(nil, BinaryOperatorType(token.Add), left, Int(3))
	...
	_, _, handled = binOpObject(nil, BinaryOperatorType(token.Sub), left, Int(3))
	require.False(t, handled)
}

func TestBinOpObjectInDispatch(t *testing.T) {
	// `a in b` dispatches on the right operand (the container) via BinOpIn.
	ret, err, handled := binOpObject(nil, BinaryOperatorType(token.In), Int(2), Array{Int(1), Int(2), Int(3)})
	require.True(t, handled)
	require.NoError(t, err)
	require.Equal(t, True, ret)

	ret, _, handled = binOpObject(nil, BinaryOperatorType(token.In), Int(9), Array{Int(1), Int(2)})
	require.True(t, handled)
	require.Equal(t, False, ret)

	// a non-container right operand is not handled.
	_, _, handled = binOpObject(nil, BinaryOperatorType(token.In), Int(1), Int(2))
	require.False(t, handled)
}
