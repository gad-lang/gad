...
- [ ] on MatchExpr, take as just syntaxe:
  ...
  on WriteCode, put arms to new line when `ctx.Flags.Has(CodeWriteContextFlagFormatMatchStmtArmsInNewLine)` (for all to
  new lines),
  or when `NEW_LINE_CALC` (put to new lines only cases when
  `writed_line_columns + formatted_inline_writed_arm_expr_columns + current_current_arm_expr_columns > ctx.MaxColumns`),
...
- [ ] on MatchStmt with syntaxe:
  ...
  on WriteCode, split arms to new line when `ctx.Flags.Has(CodeWriteContextFlagFormatMatchStmtArmsInNewLine)` (for all
  to new lines),
  when `NEW_LINE_CALC`, put to new lines only cases when
  `writed_line_columns + formatted_inline_writed_arm_expr_columns + current_current_arm_expr_columns > ctx.MaxColumns`
  like:
  ```gad
  match Expr { 
...
- [x] add doc for gad code conventions:
  ...
  ```
  apply this rule for related CodeWriter when `ctx.Flags.Has(CodeWriteContextFlagFormat*InNewLine)` (force all to new
  lines) or `NEW_LINE_CALC`
