...
// This file recreates the upstream gadx v1 `Test_RunPrintLines`
...
const compPrintLines = "@comp print_lines(rows)\n" +
	...
	"\t\t\t$el += (custom ? (*args) => custom(print_line, *args) : print_line)(i, line)\n" +
	"\t\t\t~~\n"
...
func printLinesExpect(t *testing.T, name, tpl, want string) {
	t.Helper()
	got := strings.TrimSpace(renderGadx(t, tpl, gad.Dict{}))
	if got != want {
