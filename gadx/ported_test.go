...
// This file ports the behavioural tests from the upstream gadx v1 suite
// (run_test.go / gadx_test.go at cb3a3d9) to the current API and syntax:
...
// portRun renders tpl (written to a temp .gadx file), with optional extra .gadx
...
func portRun(t *testing.T, tpl string, globals gad.Dict, modules map[string]string) (string, error) {
	...
	for name, src := range modules {
		if err := os.WriteFile(filepath.Join(dir, name+".gadx"), []byte(src), 0644); err != nil {
			t.Fatal(err)
		}
	}
