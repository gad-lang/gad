...
func firstTag(file *gadxnode.File) *gadxnode.TagStmt {
	var walk func(stmts node.Stmts) *gadxnode.TagStmt
	walk = func(stmts node.Stmts) *gadxnode.TagStmt {
		for _, s := range stmts {
			switch n := s.(type) {
...
func attrNames(tag *gadxnode.TagStmt) []string {
...
func parseTagFile(t *testing.T, src string) (*source.FileSet, *gadxnode.TagStmt) {
	t.Helper()
