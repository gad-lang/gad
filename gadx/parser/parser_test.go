...
func parseLine(t *testing.T, src string) *gadxnode.File {
	t.Helper()
...
func expectStmtCount(t *testing.T, file *gadxnode.File, n int) {
	t.Helper()
	if len(file.Stmts) != n {
...
func expectCodeStmt(t *testing.T, file *gadxnode.File, idx int, contains string) {
	t.Helper()
	if idx >= len(file.Stmts) {
		t.Fatalf("statement index %d out of range (len=%d)", idx, len(file.Stmts))
	}
	cs, ok := file.Stmts[idx].(*gadxnode.CodeStmt)
	if !ok {
		t.Fatalf("expected *gadxnode.CodeStmt at index %d, got %T", idx, file.Stmts[idx])
	}
...
func TestImportBare(t *testing.T) {
	file := parseLine(t, `@import "components.gadx"`)
	expectStmtCount(t, file, 1)
	expectCodeStmt(t, file, 0, "import")
}

func TestImportNamed(t *testing.T) {
	file := parseLine(t, `@import "components.gadx" as comps`)
	expectStmtCount(t, file, 1)
	expectCodeStmt(t, file, 0, "import")
...
func TestImportDestructureSingle(t *testing.T) {
	file := parseLine(t, `@import { page_wrapper } from "comps.gadx"`)
	expectStmtCount(t, file, 1)
...
func TestImportDestructureMultiple(t *testing.T) {
	file := parseLine(t, `@import { page_wrapper, hero, post_card } from "comps.gadx"`)
	expectStmtCount(t, file, 1)
	code := codeStmtStr(t, file, 0)
...
func globalNames(t *testing.T, gs *gadxnode.GlobalStmt) []string {
	t.Helper()
	if gs.Decl == nil {
		return gs.Names
	}
...
func TestGlobal(t *testing.T) {
	...
	gs, ok := file.Stmts[0].(*gadxnode.GlobalStmt)
	if !ok {
		t.Fatalf("expected *gadxnode.GlobalStmt, got %T", file.Stmts[0])
	}
...
func TestGlobalCommaAndDefaults(t *testing.T) {
	...
	gs := file.Stmts[0].(*gadxnode.GlobalStmt)
	names := globalNames(t, gs)
...
func TestVar(t *testing.T) {
	...
	vs, ok := file.Stmts[0].(*gadxnode.VarStmt)
	if !ok {
		t.Fatalf("expected *gadxnode.VarStmt, got %T", file.Stmts[0])
	}
	if len(vs.Decls) != 3 {
...
func TestConst(t *testing.T) {
	...
	cs, ok := file.Stmts[0].(*gadxnode.ConstStmt)
	if !ok {
		t.Fatalf("expected *gadxnode.ConstStmt, got %T", file.Stmts[0])
	}
	if len(cs.Decls) != 3 {
...
func TestImportDestructureWithMain(t *testing.T) {
	src := `@import { page_wrapper } from "comps.gadx"
@main
    +page_wrapper("Test")
        p Hello`
...
func codeStmtStr(t *testing.T, file *gadxnode.File, idx int) string {
	t.Helper()
	if idx >= len(file.Stmts) {
		t.Fatalf("statement index %d out of range (len=%d)", idx, len(file.Stmts))
	}
