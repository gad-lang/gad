...
func TestParamParse(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			var ps *gadxnode.ParamStmt
			for _, s := range file.Stmts {
				if p, ok := s.(*gadxnode.ParamStmt); ok {
					ps = p
					break
				}
			}
...
// TestParamWriteGadx checks that @param round-trips through WriteGadx.
func TestParamWriteGadx(t *testing.T) {
	tests := []struct {
		src  string
		want string
	}{
