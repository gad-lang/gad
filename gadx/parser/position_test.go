...
func parseFileWith(t *testing.T, src string) (*source.FileSet, *source.File, *gadxnode.File) {
	t.Helper()
...
func findFirstCode(file *gadxnode.File) *gadxnode.CodeStmt {
	var walk func(stmts node.Stmts) *gadxnode.CodeStmt
	walk = func(stmts node.Stmts) *gadxnode.CodeStmt {
		for _, s := range stmts {
			switch n := s.(type) {
...
// gadx source line, rather than a fragment-local line 1.
