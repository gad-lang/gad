...
func TestVarConstDeclarations(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			if got := renderGadx(t, tc.src, gad.Dict{}); got != tc.want {
...
// compile error, and the error resolves to the gadx source line.
func TestConstRequiresValue(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "t.gadx:2") {
		t.Fatalf("expected error at t.gadx:2, got: %v", err)
	}
}
