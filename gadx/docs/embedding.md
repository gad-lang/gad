...
Gadx is embedded through Gad bytecode compilation and VM execution.
...
tree (a `gadx.Element`) that you write with `WriteTo`. The `Render` struct below
does this for you.
...
`gadx.Render` provides a ready-to-use template engine with bytecode caching and
...
r.TranspilePath = func(src string) string {
    return strings.TrimSuffix(src, ".gadx") + ".gad"
}
```
...
```go
var out bytes.Buffer
err := r.Render(&out, "template.gadx", gad.Dict{
...
Do not call `gadx.AppendBuiltins(gad.NewBuiltins())` separately for compile and
run steps.
...
`.gadx` files natively. The importer is set via `gad.ModuleMap.SetExtImporter`:
...
`FileImporter` also handles named imports (`@import "file.gadx" as name`) and
compiles imported `.gadx` files to Gad bytecode transparently. (`gadx.Render`
...
`gad.TranspileGadx` parses a Gadx source and writes the equivalent Gad source to
a file (a `.gad` suffix is appended when missing):

```go
import "github.com/gad-lang/gad"

err := gad.TranspileGadx("template.gadx", src, "out/template.gad")
```
