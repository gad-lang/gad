...
Gadx preserves the position (file, line, column) of the original `.gadx` source
...
A gadx template is transpiled to Gad and then compiled and executed. When a
...
NotCallableError: nil
    at layouts/index.gadx:10:5
       layouts/default.gadx:50:32
```
...
1. **The source file's line table must be populated.** Gad resolves a byte
   offset to a line/column using `source.File.Lines`. The gadx scanner reads the
...
2. **Embedded Gad fragments must be parsed in the source coordinate space.**
   Gadx extracts Gad code from directives and parses it with Gad's parser:
