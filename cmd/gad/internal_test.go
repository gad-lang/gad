...
`)
	...
	t.Run("modules_cache", func(t *testing.T) {
		...
		require.Equal(t, string(cw.consume()), "[<nil>]\n")
		require.NoError(t, r.execute("test := 2"))
		cw.consume()
		require.NoError(t, r.execute(".modules_cache"))
		require.Equal(t, string(cw.consume()), "[<nil>]\n")
		require.NoError(t, r.execute("@module"))
		cw.consume()
		require.NoError(t, r.execute(".modules_cache"))
		require.Equal(t, string(cw.consume()), "[‹module: (repl)›]\n")
	})
	...
	t.Run("type_method_constructor", func(t *testing.T) {
		...
		require.Equal(t, "⇦   \"‹builtin type ‹int› with 9 methods›\"",
