...
// runScript compiles and runs src (Gad by default; Giom when giom is set) with
// the giom builtins registered, returning the return value and captured stdout.
// When the return value is a giom.Element (a rendered template tree), its HTML is
// written to the captured stdout so callers can assert on the rendered output.
func runScript(t *testing.T, src string, giomMode bool) (gad.Object, string) {
	t.Helper()
...
func TestNativeGiomCompile(t *testing.T) {
	// A .giom template compiled through gad.CompileOptions.GiomOptions returns a
	// giom.Tag; runScript writes its rendered HTML into the captured output.
	_, html := runScript(t, `p Hello {= 1 + 2 }`, true)
	if !strings.Contains(html, "Hello") || !strings.Contains(html, "3") {
		t.Fatalf("giom render = %q, want it to contain Hello and 3", html)
	}
}
...
func TestGadParseGiomEval(t *testing.T) {
	// gad.parse with type=gad.SourceType.GIOM returns lowered statements
	// referencing giom builtins; gad.eval runs them (the VM has the giom
	// builtins registered) and returns the giom element, which runScript renders.
	_, html := runScript(t, `
		src := gad.parse("span Hi"; type=gad.SourceType.GIOM)
		return gad.eval(src.stmts)
	`, false)
	if !strings.Contains(html, "Hi") {
		t.Fatalf("giom eval = %q, want it to contain Hi", html)
	}
}
