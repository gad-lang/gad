...
and customising them with `met gad.unOp{Op}`.

Every unary operator is dispatched through its `gad.unOp{Op}(operand)` builtin
(e.g. `gad.unOpSub`); the VM applies one to the top of the stack, and a type can
override it.
