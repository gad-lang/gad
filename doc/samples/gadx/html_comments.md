...
- **`<!-- … -->`** is a comment, and it is written into the page: that is how a
  template hands a note to whatever reads the rendered HTML. It nests nothing,
  so a `/` inside it (`js/main.js`) is text, not a self-closing tag. A note for
  the template itself, one that stops here, is a `//` line comment.
- **`<!DOCTYPE …>`** and **`<![CDATA[ … ]]>`** are declarations. They carry
  nothing to render and are dropped. To emit a doctype, use the `!!! 5`
  statement, which renders `<!DOCTYPE html>`.
...
@main
    ...
    <html lang="en">
    <head>
    <!-- A comment goes out as it stands, whatever it holds: a/b.js, {braces}. -->
    <meta charset="UTF-8">
