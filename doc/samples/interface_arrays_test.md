...
# Slice interfaces (`interface P [] { … }`)

Writing one or more `[]` after the interface's name makes the
[interface](interfaces.gad) match an **array** — a *slice interface*:
...
- `interface P [] { … }` matches a flat array whose elements each satisfy the
  ...
  the same.
- `interface P []<int|uint>` (or `interface P []int`) is a **slice of types**:
...
converts too. A slice interface is also a parameter type, carries
[metadata](metadata.gad) and answers the reflection keys `@depth` (the number of
`[]`), `@elem` (the element types of a slice of types, else nil) and `@meta`.
An empty array satisfies any slice interface vacuously.
...
/**
`interface P []<T1|T2>` is a slice of **types**: every leaf must match one of the
...
/**
A slice interface is a type like any other, so it works as a parameter type: the
...
arrays. A typed array converts to a slice interface (and back) the same way.
**/
test "transforming an array" {
    ...
    // A typed array converts to a slice interface value (a plain array), each
...
/**
Reflection: `@depth` is the number of `[]`, `@elem` the element types of a slice
