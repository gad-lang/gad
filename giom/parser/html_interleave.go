...
// sentinel is the single byte a giom block is collapsed to inside the rewritten
...
// htmlSubError carries an error from a recursively parsed inline giom block back
...
// isGiomMarker reports whether a line whose leading whitespace has been stripped
// (rest) begins a block-level giom statement that may be interleaved inside an
...
func isGiomMarker(rest string) bool {
	if rest == "" {
		return false
	}
	switch rest[0] {
...
// isGiomContinuation reports whether a same-indent line continues the preceding
// giom block rather than starting a new one — i.e. an `@else` / `@else if`
// clause of an enclosing `@if`/`@for`.
func isGiomContinuation(rest string) bool {
	return strings.HasPrefix(rest, "@else")
}

// rewriteGiomBlocks finds block-level giom statements interleaved in a raw HTML
...
// Every extracted block is parsed with the real giom parser — so it may itself
...
func rewriteGiomBlocks(raw string, base source.Pos) (string, []gnode.Stmts, []htmlSubError) {
	...
	for li := 0; li < len(lines); li++ {
		...
		for j := li + 1; j < len(lines); j++ {
			...
			if w == gIndent && isGiomContinuation(strings.TrimLeft(lines[j].text, " \t")) {
				last = j
				continue
			}
			break
		...
		stmts, err := subParseGiom(sb.String(), subBase)
		if err != nil {
...
// subParseGiom parses an inline giom block (already dedented) with the giom
// parser, aligning the fragment file's base to base so node positions resolve
// back onto the original .giom source (line-accurate; columns land at the start
// of the dedented line).
func subParseGiom(text string, base source.Pos) (gnode.Stmts, error) {
	fs := source.NewFileSet()
	fbase := -1
	if base != noBase && int(base) >= fs.Base {
		fbase = int(base)
	}
	f := fs.AddFileData("(giom-inline)", fbase, []byte(text))
